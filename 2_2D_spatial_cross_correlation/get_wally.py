        ...
        if (left_edge > 0 and right_edge > 0):
            ...
            if (right_match & left_match):
                ...
                for indices in search_area:
                    gs_temp = ConvertRGBAtoGreyscale(template_array[r_bottom_maze + indices[0]:r_top_maze + indices[0], 
                                                     c_left_maze + indices[1]:c_right_maze + indices[1], 
                                                     :])
                    gs_pat = ConvertRGBAtoGreyscale(pattern_array)
                    cc = CalcNormedCrossCorr2D(gs_temp, gs_pat)
                    if ccmax < cc:
                        ccmax = cc
                        coords = (r_bottom_maze + indices[0], 
                                  r_top_maze + indices[0], 
                                  c_left_maze + indices[1], 
                                  c_right_maze + indices[1])
