...
def GetFrames(raw_video_path=os.path.join(os.getcwd(),'6_project/data/raw/videoplayback'),
              start_frames=7110,
              end_frames=45203):
    vidcap = cv2.VideoCapture(raw_video_path)

    # get frames
    count = 0
    while success:
        if count > start_frames and count < end_frames:
            success,image = vidcap.read()
            cv2.imwrite("6_project/data/interim/frame%d.jpg" % count, image)
            print(f'{count} frame saved!')
            if cv2.waitKey(10) == 27:
                break
        count += 1


def SplitImages(interim_frame_path = os.path.join(os.getcwd(),'6_project/data/interim/'),
                processed_frame_path = os.path.join(os.getcwd(),'6_project/data/processed/')):

    all_frames = os.listdir(interim_frame_path)

    for frame in all_frames:
        name = re.findall('frame\d+',frame)
        name_left = name[0] + '_left.jpg'
        name_right = name[0] + '_right.jpg'

        im_path = os.path.join(interim_frame_path,frame)
        im = Image.open(im_path)
        im = np.asarray(im)
        im = ConvertRGBAtoGreyscale(im)  

        left = im[65:280,10:310]
        # img = Image.fromarray(left)
        # img.show()
        right = im[65:280,330:-10]
        # img = Image.fromarray(right)
        # img.show()

        left_im = Image.fromarray(left)
        left_im = left_im.convert("L")
        left_im.save(os.path.join(processed_frame_path,name_left))

        right_im = Image.fromarray(right)
        right_im = right_im.convert("L")
        right_im.save(os.path.join(processed_frame_path,name_right))
...
def SplitAxis(im, num_pixels=50, overlap=0, axis=0):
    ...
    try:
        ...
        print("overlap is zero, or must be a fraction between (0,1). Assume overlap = 0")
        ...
        # print(f'x-overlaps: {a_overlap_shifts}')
    
    try:
        assert axis == 0 or axis == 1
    except AssertionError:
        print("axis value needs to be 0 or 1")
    else:
        a_max = im.shape[axis]
        # print(f'length of axis: {(a_max)}')

        a_splits = np.arange(0, num_pixels + a_max, num_pixels) # cols
        a_splits[-1] = a_max
        # print(f'x breaks: {a_splits}')
...
def GetWindowSearchArea(im, rs, re, cs, ce, padding=0):
        ...
        # print(f'search area cols: {col_search} search area rows: {row_search}')
...
def GetFlatSearchArea(im, rs, re, cs, ce, padding=0):
        ...
        # print(f'right search area cols: {col_search} and search area rows: {row_search}')
